# tools/koutsuhi.py (ハイブリッドAI搭載型 - 最終決戦仕様)
...
# ------------------------------------------------
# ★★★ ここが、2つのAPIキーを管理する、新しい心臓部です ★★★
# ------------------------------------------------
def get_user_api_keys():
    """
    ユーザーのブラウザから、MapsとGemini、両方のAPIキーを取得・管理する。
    戻り値: (maps_key, gemini_key) のタプル。片方でも欠けていれば (None, None)
    """
    localS = LocalStorage()
    ...
    with st.sidebar:
        ...
        if maps_key and gemini_key:
            ...
            if st.button("🔄 APIキーを再設定する"):
                ...
                localS.setItem("user_gemini_api_key", None)
                st.rerun()
            ...
            with st.form("api_keys_form"):
                ...
                if submitted:
                    if not new_maps_key or not new_gemini_key:
                        ...
                        localS.setItem("user_gmaps_api_key", {"value": new_maps_key.strip()})
                        localS.setItem("user_gemini_api_key", {"value": new_gemini_key.strip()})
...
def generate_ai_transit_summary(gemini_key, directions_result):
    try:
        ...
        3.  各ステップは、`travel_mode`に応じて、以下の形式で、絵文字を使いながら表現してください。
            - **TRANSIT（公共交通機関）の場合:**
              - `line.name`(路線名)と`line.vehicle.name`(乗り物の種類)を表示してください。
              - `departure_stop.name`(出発駅)と`arrival_stop.name`(到着駅)、そして`num_stops`(駅数)を必ず含めてください。
            - **WALKING（徒歩）の場合:**
              - `html_instructions`(指示)と`distance.text`(距離)を簡潔にまとめてください。
    ...
    if maps_key and gemini_key:
        ...
        if submit_button:
            ...
            with st.spinner("🤖 2人のAIアシスタントが、協力して最適なルートを検索中です..."):
                try:
                    # ステップ1：地図の専門家（Maps API）に、正確なデータを尋ねる
                    ...
                    if not directions_result:
                        st.error("❌ 指定された場所間の公共交通機関ルートが見つかりませんでした。"); return

                    # ステップ2：コミュニケーションの専門家（Gemini）に、データの翻訳と要約を依頼する
                    summary, error = generate_ai_transit_summary(gemini_key, directions_result)
                    
                    if error:
                        st.error(error)
                        with st.expander("元のAPIデータを見る"):
